...
var webAddress=require('./config').webAddress;
    ...
    if (req.query.action === 'config') {
      ...
      busboy.on('file', function (fieldname, file, filename, encoding, mimetype) {
        ...
        res.ue_up = function (img_url) {
          ...
          console.log(static_url+","+img_url+","+name);
          ...
          writeStream.on("close", function () {
            fse.move(tmpdir, dest, function (err) {
              if (err) throw err;
              var obj={
                'url': host+"upload/"+path.join(img_url, name).replace(/\\/g,'/'),
              ...
              // .json({
              //   'url': "http://localhost:3008/upload/"+path.join(img_url, name).replace(/\\/g,'/'),
              //   'title': req.body.pictitle,
              //   'original': filename,
              //   'state': 'SUCCESS'
              // });
